<template>
  <div class='patientInfo'>
    <div class='bar'>
      <div class='left-bar'>
        <div class='number'>
          病人总览  <span>{{datalist.length}}</span> 人
        </div>
        <select class='patientnum' name="patientnum" id="patientnum" @click='patientSelClick'>
          <option value="所有病人">所有病人</option>
          <option v-for='(item, index1) in patientname' :key='index1' :value="item">{{item}}</option>
        </select>
        <select class='state' name="state" id="state" @click='stateSelClick'>
          <option value="全部状态">全部状态</option>
          <option v-for='(item, index2) in patientstate' :key='index2' :value="item">{{item}}</option>
        </select>
        <div class='search'>
          <input class='search-inp' type="text" placeholder="查找姓名、编号、就诊卡号、医保卡" />
    ...
    <div class='patientdata'>
      <table class='datalist'>
        ...
        <tr v-for='(item,index3) in datalist' :key='index3' >
          ...
          <td>{{item.state}}</td>
          <td>{{item.date}}</td>
          <td @click='patientItemClick(index3)' class='look'>查看</td>
        </tr>
...
export default {
  ...
  methods:{
    getPatientData() {
      this.$axios.post('http://localhost:5000/patientdata', {page: this.page}).then(res =>{
    ...
    searchClick() {
      ...
      for(let i = 1; i < trs.length; i++) {
        trs[i].style.display = 'none'
        if(input.value === '') {trs[i].style.display = ''}
        if(input.value === trs[i].children[0].innerHTML || input.value === trs[i].children[2].innerHTML || input.value === trs[i].children[4].innerHTML) {
          trs[i].style.display = ''
        }
        
      }
    },
    patientItemClick(index3) {
